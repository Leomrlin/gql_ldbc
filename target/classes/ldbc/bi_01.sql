--博文摘要
--描述
--将时间范围内Msg按年份、类型、长度组依次分组
--聚合出消息数、平均消息长度、消息长度和、所占百分比
--排序要求：年份降序，类型升序，长度组升序
--参数：$date Date = 1696160400000
--输出：year bigint, isComment boolean, lengthCategory bigint, messageCount bigint,
--     averageMessageLength double, sumMessageLength bigint, percentageOfMessages double
--线上参数： $date Date = 1326269831000
--线上参数： $date Date = 1293172663000
CREATE GRAPH bi (
  --static
  --Place
  Vertex Country (
